...
import ImpImage from '../Model/ImpImage.js'; // Correct import path
...
router.post('/uploadImp/:id', upload.array('images', 10), async (req, res) => {
    try {
...
router.get('/imimage/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const images = await ImpImage.findAll({
            where: { book_id: id },
        });
...
// Delete image by id
router.delete('/deleteimp/:id', async (req, res) => {
    const { id } = req.params;
    console.log(`Received request to delete image with id: ${id}`); // Debugging line
    try {
        const image = await ImpImage.findByPk(id);
        if (!image) {
