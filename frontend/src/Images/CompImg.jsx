...
const CompImg = () => {
  ...
  const fetchImages = useCallback(async () => {
    try {
      const response = await axios.get(`http://localhost:8081/api/images/compimage/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`http://localhost:8081/api/images/uploadComp/${id}`, formData, {
        headers: {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8081/api/images/deletecomp/${id}`);
